...
WAN 和 MiniMax 必须由使用者先完成自己的工具连接、模型选择和授权。Google Flow 的作用是把视觉源、动作提示和单镜信息整理成可操作的手动包；生成、下载和实际账户操作仍由使用者完成。
...
已提交任务、历史回执和旧输出不会因为换渠道而被抹掉或改写。切换渠道也不等于重启内容、换画风或允许额外费用。
